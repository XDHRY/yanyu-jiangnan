...
- **AI 贴图**：7 张 albedo 贴图（粉墙/苔石/梅皮/黛瓦/老木/花瓣/远山夜景）由 image_gen 按存档提示词生成，已 pack 进 `.blend`。
- **可验证交付**：`tools/verify_render.py` 内置 7 项断言（天气互斥、帧间动态、驱动器有效性、贴图打包、法线朝向等）。

场景规模：2745 对象（1323 网格 + 1367 曲线）/ 13.3 万顶点 / 23 材质 / 11 集合 / 3 机位 / 11 灯光（详见 `validation/scene_statistics.json`）。

## 快速开始

1. 用 **Blender 4.5 LTS**（其他 4.x 版本未验证）打开根目录 `Jiangnan.blend`，选择场景「烟雨江南 · 雪月江南」。
...
```
├── Jiangnan.blend     # 主场景（Jiangnan.blend1 为 Blender 自动备份快照）
